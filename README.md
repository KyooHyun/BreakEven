...
경기도 카드 소비 집계 데이터와 여신금융협회 가맹점 수수료율 공시로, 카드 혜택이 손익 위에서 성립하는 지점을 역산한 프로젝트. 기획자 1인, 데이터 분석·개발 담당(본인) 1인으로 진행했다. 상세 기획은 [PROJECT_SPEC.md](PROJECT_SPEC.md) 참조.
...
`src/04_economics.py`가 뱉는 **스크립트 원본 출력**이고, 아래 표는 그 열을 계산 근거로
...
| 상품안 | 타겟 세그먼트 | 즉시할인 업종·혜택률 | g*(즉시할인) | 피크 시간대 | 설계 근거 |
|---|---|---|---|---|---|
| cluster0 — 금·13-15시 고액결제형 | F/M 60대 이상 | 소매/유통 0.3%, 의료/건강 0.3% | 35.3% / 42.9% | 금 13-15시 | 상위 지출 3개 중 2개가 threshold를 통과한 유일한 세그먼트. 3위 음식(지출 16.2%, f=0.40%)은 r=0.3%에서도 g*=300%라 제외 |
| cluster1 — 토·17-19시 소액결제형 | F/M 10~20대 | 소매/유통 0.3% | 35.3% | 토 17-19시 | 소매/유통(37.0%)만 통과. 2위 음식(29.1%)은 이연적립으로도 g*=110.5%에 그쳐 미충족 — 세 세그먼트 중 혜택 공백이 가장 큰 구성 |
| cluster2 — 토·15-17시 중간결제형 | F/M 30~60대 | 소매/유통 0.3% | 35.3% | 토 15-17시 | 최대 볼륨(월 1.4억 건). 2위 음식(19.0%)은 연회비 풀의 약 20% 배정으로 threshold 충족 — 볼륨이 커 연회비 풀 자체가 큰 것이 이유 |
...
**탄력성 가정을 반영하면 위 표의 판정 자체가 달라진다** — 세그먼트별 혜택 반응
탄력성(근거 얇은 판단치)을 적용하면 세 상품안의 소매/유통 혜택(threshold 기준으로는 전부
성립)이 탄력성 기준으로는 **전부 탈락**한다. 즉 이 표의 g*(즉시할인) 열은 threshold
기준일 뿐이고, 실제 달성 가능성까지 보장하지 않는다.
(근거: outputs/tables/bep_heatmap.csv, docs/hypothesis-log.md H7)
...
데이터에 부정당한 셈이다. 분석에서 필요한 건 정교한 가설을 세우는 쪽이
...
마지막에 알게 된 건 식 자체에 빈 자리가 있었다는 것이다. 내가 쓰던 f는 명목
가맹점 수수료율이었는데, 독자결제망과 대행망을 함께 쓰는 카드사는 결제가 어느
망을 타느냐에 따라 대행 수수료가 붙고 안 붙는다. 그 항이 내 식에는 없었다.
넣고 보니 지금까지의 계산 전체가 대행 수수료 0, 즉 전환이 이미 끝난 상태를
가정하고 있었다. 실측 대리값 37.8%를 넣으면 명목 f의 13~15%가 깎인다.

그런데 이 결과는 한 덩어리가 아니었다. 잠식의 크기는 threshold와 무관하게
성립하지만, 그 크기 때문에 판정이 뒤집힌다는 결론은 threshold를 52%로만 올려도
사라진다. 내가 정한 50%가 경계에서 1.8%p 아래에 있었던 것이다. 하나의 발견처럼
보이던 게 서로 다른 가정에 기대는 두 층이었다.

방향도 예상과 달랐다. 새어나가는 금액이 가장 큰 업종은 전환해도 판정이 바뀌지
않고, 판정이 실제로 뒤집히는 업종은 금액이 작았다. 비용을 아끼는 일과 혜택
설계가 풀리는 일이 같은 업종에서 일어나지 않는다. 다만 이 계산 전체가 대행
수수료율 c에 걸려 있는데, 그 값은 공시에서 분리되지 않아 네 경로를 시도하고도
특정하지 못했다.

정리하면 이 프로젝트가 내놓은 건 좋은 카드 상품 하나가 아니라, 혜택을 주려면
무엇이 먼저 바뀌어야 하는지에 대한 계산된 목록이다. 유입형 상권을 의심한
것부터 c를 특정하지 못했다고 적은 것까지, 일관되게 지키려 한 기준은
